...
export function parseTimestampText(text: string): { timestamps: Timestamp[]; errors: ValidationError[] } {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    try {
      ...
      errors.push({
        line: i + 1,
        message: error instanceof Error ? error.message : 'パース中にエラーが発生しました',
        type: 'format'
      });
    }
  }
...
function parseTimestampLine(line: string, lineNumber: number): Timestamp {
  ...
  if (!match) {
    throw new Error('タイムスタンプの形式が正しくありません。形式: "開始時間 ~ 終了時間 タイトル"');
  }
...
function parseTimeString(timeStr: string): number {
  ...
  throw new Error('時間形式が正しくありません。MM:SS または HH:MM:SS 形式で入力してください');
}
