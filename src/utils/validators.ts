...
export function validateFile(file: File): ValidationError[] {
  ...
  // 対応形式チェック
  const supportedTypes = ['audio/mpeg', 'audio/mp4', 'audio/m4a', 'audio/aac', 'audio/ogg', 'audio/wav'];
  ...
  if (!isTypeSupported && !isExtensionSupported) {
    errors.push({
      line: 0,
      message: 'サポートされていないファイル形式です。MP3、M4A、AAC、OGG、WAVファイルを選択してください',
      type: 'format'
    });
  }
...
export function validateTimestampsAgainstDuration(
  ...
  for (let i = 0; i < timestamps.length; i++) {
    ...
    if (timestamp.startTime > duration) {
      errors.push({
        line: i + 1,
        message: `開始時間がファイル長(${Math.floor(duration)}秒)を超えています`,
        type: 'duration'
      });
    }
    ...
    if (timestamp.endTime > duration) {
      errors.push({
        line: i + 1,
        message: `終了時間がファイル長(${Math.floor(duration)}秒)を超えています`,
        type: 'duration'
      });
    }
  }
...
export function getErrorsByType(errors: ValidationError[], type: ValidationError['type']): ValidationError[] {
