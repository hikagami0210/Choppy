...
export async function extractMetadata(file: File): Promise<AudioMetadata | null> {
  try {
...
export function createSegmentMetadata(
  ...
  const segmentMetadata: AudioMetadata = {
    ...originalMetadata,
    title: originalMetadata.title ? `${originalMetadata.title} - ${segmentTitle}` : segmentTitle
  };
